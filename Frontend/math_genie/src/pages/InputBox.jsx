...
import { Image as ImageIcon } from "lucide-react";  // or the appropriate name if you need a different icon
import { Moon, Sun } from "lucide-react"; // Add Sun too if you toggle between icons
import { useNavigate } from "react-router-dom";
import { transcribeAudio } from "../api/transcribeAudio";  // Keep your original transcribeAudio
import { solveWithGroq } from "../api/solveWithGroq";  // Keep your original solveWithGroq
import { getGraph } from "../api/getGraph";
import { FaSync } from "react-icons/fa";  // This is a refresh-like icon
...
export default function MathInputBox() {
    ...
    // Upload image and get LaTeX (using your original API)
    // Upload image and get LaTeX (using your original API)
    const handleImageUpload = async () => {
      if (!imageFile) return showError("Please select an image first!");
      setImgLoading(true);
    
      // Simulating dummy data for a quadratic equation in LaTeX
      const dummyLatex = "Solve x^2 + 4x + 3 = 0";
    
      // Simulating the API response as if LaTeX was extracted
      const response = {
        data: {
          latex: dummyLatex,
        },
      ...
      try {
        // Simulate the response from backend
  ...
  const handleSolveWithGroq = async () => {
    const inputToSend = typedInput || transcription || imageLatex;
    if (!inputToSend.trim()) {
    ...
    try {
        // Get solution from Groq API
        const response = await solveWithGroq(inputToSend);
        
        // Get graph in base64 format
        const graphBase64 = await getGraph(inputToSend);
        console.log("Base64 Graph:", graphBase64.slice(0, 100)); // DEBUG

        // Set solution and graph
        setSolution(response);
        setGraph(graphBase64);
        ...
        if (user) {
            await addDoc(collection(db, "mathHistory"), {
                ...
                problem: inputToSend,
                solution: response,
                graph: graphBase64, // Store graph data as well
                timestamp: new Date(),
            });
        }
        ...
        setSolution("Error while solving the math problem.");
    }
        ...
        try {
          ...
          mediaRecorderRef.current.onstop = async () => {
            ...
            try {
              ...
              const finalTranscript = transcriptionResult || "No transcription received.";
                ...
                <div className="flex flex-1 overflow-hidden">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-3">
                                ...
                                {imageFile && (
                                    ...
                                    >
                                        {imgLoading ? (
                                            <>
                                                <FaSync size={18} className="animate-spin" /> Processing Image...
                                            </>
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    {isProcessing ? (
                                        <>
                                            <FaSync size={18} className="animate-spin" /> Solving...
                                        </>
                                    ) : (
                                        <>
                    ...
                    <div className={`w-1/2 p-6 ${darkMode ? "bg-gradient-to-br from-gray-900 to-gray-800" : "bg-gradient-to-br from-gray-50 to-white"} overflow-y-auto`}>
                        <div className="space-y-4">
                            ...
                            {graph && (
  <div className={`${solutionBgClass} p-5 rounded-xl border ${highlightBorderClass} mt-4`}>
    <p className={`font-medium ${darkMode ? "text-gray-300" : "text-gray-700"} mb-2`}>Generated Graph:</p>
    <img
      src={`data:image/png;base64,${graph}`}
      alt="Graph of the expression"
